                        ...
                        <tbody class="bg-white dark:bg-zinc-900 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($imagenes as $imagen)
                                <tr wire:key="fila-{{ $imagen->id }}" class="hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ \Illuminate\Support\Str::limit($imagen->descripcion, 40) }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ optional($imagen->fecha_subida)->format('d/m/Y H:i') ?? '-' }}</td>
